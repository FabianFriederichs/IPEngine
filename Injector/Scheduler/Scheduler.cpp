...
Scheduler::Scheduler() :
	m_idgen(0),
	m_subscriptions(),
	m_pool(std::thread::hardware_concurrency()-3)
{
...
Scheduler::SubHandle Scheduler::subscribe(const function<void(TaskContext&)>& schedfunc, interval_t desiredInterval, SubType type, float timescale)
{	
	SchedSub* sub = new SchedSub(m_idgen.fetch_add(1, std::memory_order_relaxed),
		m_pool.createTask(schedfunc, TaskContext(SchedInfo())),
		type,
		desiredInterval,
		timescale);
	std::unique_lock<YieldingSpinLock<5000>> lock(m_sublock);
