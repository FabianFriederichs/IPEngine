...
public:
	...
	int shadowDescrSetIndex = -1;
	int geomDescrSetIndex = -1;
	MaterialType_t materialType = MATERIAL_TYPE_FSCHLICK_DGGX_GSMITH;


	//static void loadFromGLTF(std::unordered_map<ipengine::ipid, VMesh> &retMeshes, rj::VManager *pManager, const std::string &gltfFileName,
	...
	//		{
	//			retMeshes.emplace( 0, pManager  );
	//			//retMeshes.emplace_back(pManager);
	//			auto &retMesh = retMeshes.end()->second;
